...
namespace UltimateReplay
{
    ...
    public class ReplayObjectInspector : ReplayRecordableBehaviourInspector
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            // Check for id assign mode
            if (IsAnyTargetAReplayPrefabAsset() == true)
                EditorGUILayout.PropertyField(serializedObject.FindProperty("instantiateIdentity"));

            // Display properties
            EditorGUILayout.PropertyField(serializedObject.FindProperty("replayIdentity"));

            // Check for prefab id
            if(IsAnyTargetAReplayPrefab() == true)
            ...
            else
            {
                GUILayout.BeginHorizontal();
                {
                    ...
                    IReadOnlyList<ReplayBehaviour> observed = targetReplayObject.ObservedComponents;
                    ...
                    {
                        // Display foldout
                        targetReplayObject.isObservedComponentsExpanded = GUILayout.Toggle(targetReplayObject.isObservedComponentsExpanded, "Replay Components", EditorStyles.foldout, GUILayout.Width(EditorGUIUtility.labelWidth - 2));
        ...
        private bool IsAnyTargetAReplayPrefab()
        {
            foreach(ReplayObject obj in targetReplayObjects)
            {
                if (obj.IsPrefab == true)
                    return true;
            }
            return false;
        }

        private bool IsAnyTargetAReplayPrefabAsset()
        {
            foreach (ReplayObject obj in targetReplayObjects)
            {
                if (obj.IsPrefab == true && obj.IsPrefabAsset == true)
                    return true;
            }
            return false;
        }
