...
namespace UnityStandardAssets.Vehicles.Car
{
    ...
    public class CarAIControl : MonoBehaviour
    {
        ...
        public Material bodyMaterialDamaged1;
        public GameObject carBody;

        public GameObject blueColor, redColor, blackColor;
        public Material damagedBlue, damagedRed, damagedBlack;
        private void Awake()
        {
        ...
        private void FixedUpdate()
        {
            ...
            if(m_Rigidbody.velocity.magnitude<3)
            {
                ...
                if(currentTimer>8)
                {
                    GameObject closestGameObject = FindClosestGameObjectInArray(markers, transform.position);
                    transform.position = closestGameObject.transform.position;
                    currentTimer = 0;
                }
                //Debug.Log("ads");
            }
        ...
        GameObject FindClosestGameObjectInArray(GameObject[] objects, Vector3 targetPos)
        {
            ...
            private void OnCollisionStay(Collision col)
        {
            ...
            if (col.rigidbody != null)
            {
                ...
                {
                    Debug.Log("sa");
                    carBody.GetComponent<MeshRenderer>().material = bodyMaterialDamaged1;
                ...
                var othergamboject = col.rigidbody.GetComponent<CarUserControl>();
                if(othergamboject!=null)
                {
                   if(blueColor.activeInHierarchy==true)
                    {
                        blueColor.GetComponent<MeshRenderer>().material = damagedBlue;
                    }
                    if (blackColor.activeInHierarchy == true)
                    {
                        blackColor.GetComponent<MeshRenderer>().material = damagedBlack;
                    }
                    if (redColor.activeInHierarchy == true)
                    {
                        redColor.GetComponent<MeshRenderer>().material = damagedRed;
                    }
                }
            }
        }
